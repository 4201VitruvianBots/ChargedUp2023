...
public class AutoTest {
    ...
    public void testAutoPathFipping() {
        ...
        var blueAuto = new OnePiece("BlueOnePiece", m_autoBuilder, m_swerveDrive, m_fieldSim, m_wrist, m_intake, m_vision);
        var blueTrajectories = blueAuto.getTrajectory();
        for (var segment : blueTrajectories) {
            for (var state : segment.getStates()) {
                assert (blueTrajectoryMinX < state.poseMeters.getX() && state.poseMeters.getX() < blueTrajectoryMaxX);
                assert (blueTrajectoryMinY < state.poseMeters.getY() && state.poseMeters.getY() < blueTrajectoryMaxY);
            }
        }
...
//        var test = TrajectoryUtils.readTrajectory("RedOnePiece", new PathConstraints(1, 1));
        ...
        var redAuto = new OnePiece("RedOnePiece", m_autoBuilder, m_swerveDrive, m_fieldSim, m_wrist, m_intake, m_vision);
        var redTrajectories = redAuto.getTrajectory();
        for (var segment : redTrajectories) {
            for (var state : segment.getStates()) {
                assert (redTrajectoryMinX < state.poseMeters.getX() && state.poseMeters.getX() < redTrajectoryMaxX);
                assert (redTrajectoryMinY < state.poseMeters.getY() && state.poseMeters.getY() < redTrajectoryMaxY);
            }
        }
    }
}
