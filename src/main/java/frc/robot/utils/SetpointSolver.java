...
public class SetpointSolver {
    ...
    public void solveSetpoints(Pose2d currentRobotPose, Pose2d targetPose, Constants.SCORING_STATES scoringState) {
        solveSetpoints(currentRobotPose, targetPose, scoringState, 0);
    }

    /** Given the current robot pose and a valid target pose, find the closest pose and calculate the desired
     *  setpoints for the chassis heading and elevator height. Assume fixed angle setpoints for the wrist, which will
     *  determine the max elevator extension. Can also take into account the approximate offset of the game object
     *  within the intake.
     */
    public void solveSetpoints(Pose2d currentRobotPose, Pose2d targetPose, Constants.SCORING_STATES scoringState, double targetTangentalOffset) {
        ...
        var solutionTransform = new Transform2d(new Translation2d(0, m_targetTangentalOffset), Rotation2d.fromDegrees(90));

        var correctedSolution = solution.transformBy(solutionTransform);

        chassisSetpointDegrees = correctedSolution.getRotation().plus(scoringState == Constants.SCORING_STATES.LOW_INTAKE ?
        ...
        elevatorSetpointMeters = Math.cos(ConstantsRushHour.ElevatorDef.elevatorMountAngle.getRadians()) * elevatorHorizontalSetpointMeters;
    }
